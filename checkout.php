...
<?php
	...
	if(isset($_POST['btnOrder']))
	{
		...
		$rounded = number_format($total_amount, 2);
		//$billno_sql="select * from tblbill where customerid=$custid  and date = '$date'  and ABS(total_amount - $rounded)<0.01";
		$billno_sql = "SELECT * FROM tblbill WHERE customerid = $custid AND date = '$date' AND ABS(total_amount - $rounded) < 0.01";
		$bill_query=mysqli_query($con,$bill_sql);
		$billno_query=mysqli_query($con,$billno_sql);
		$row_count = mysqli_num_rows($billno_query);
		echo " count ";
		...
		foreach ($Data as $value)
		{
			...
			$pid2=$value['pid'];
			$qnt=$value['pqty'];
			$prd="select quntity from tblproduct where id=$pid2 ";
			$prd2=mysqli_query($con,$prd);
			while ($row2=mysqli_fetch_array($prd2)){
				$quntity=$row2['quntity'];
				$quntity=$quntity-$qnt;
				 mysqli_query($con,"UPDATE  tblproduct set quntity=$quntity  where id=$pid2");
	...
	<div class="page_wrap">
		...
		?>
		 <?php
			...
			$sql="select P.id, PC.imagepath, PC.name, M.type, P.price, C.qty, P.price*C.qty as 'total' from tblCart C inner join tblproduct P on C.productid=P.id inner join tblproductcategory PC on P.productcategoryid = PC.id inner join tblmeasurement M on P.measurementid= M.id where C.customerid=$uid";
			$cartRES=$con->query($sql);
			$total_amt=0;
		?>
												...
												<div class="column-2_3">
													<form class="sc_form" action="#" method="post">
														<table class="sc_table">
                                                            ...
                                                            echo $rounded;
                                                            $total_amt=$total_amt+$row['total'];?></label></td>																	</tr>
															...
															<tr>
																<td colspan="6">
																<label style="text-align: right;"><b>*By clicking on the <a>'Place Your Order'</a> button, you agree  <a>privacy notice</a> and <a>conditions of use</a>.</b></label>
																</td>
															</tr>
														</table>
