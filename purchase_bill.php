<?php
	...
	if (!isset($_GET['billno'])) {
		...
		while ($order = mysqli_fetch_array($order_query)) {
			...
			$total_amt = (float) str_replace(',', '', $total_amt);
		...
		$invoice->set("company", [
		"http://localhost/d4/image/logo2.png",
			"MILKY>COM",
			" Haveri, Haveri - 581110 ",
			"Phone: +91 9108296502", "Whatsapp: +91 6362510146",
			"Email: info@milky.com"
		]);
...
// 2D - SHIP TO
		...
		// $invoice->outputPDF();
		$url =  'http://localhost/d4/download/invoice'.$billno.'.pdf';
